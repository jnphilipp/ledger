...
def accounts(request, slug):
    ...
    if year:
        ...
        for account in Account.objects.filter(Q(entries__tags=tag) & Q(ledgers=ledger) & Q(entries__day__year=year)).distinct():
            series.append({
                'name': account.name,
                'data': [[m.strftime('%B'), tag.entries.filter(Q(account=account) & Q(day__year=year) & Q(day__month=m.strftime('%m'))).aggregate(sum=Sum('amount'))['sum']] for m in months],
                'tooltip': {
        ...
        for account in Account.objects.filter(Q(entries__tags=tag) & Q(ledgers=ledger)).distinct():
            series.append({
                'name': account.name,
                'data': [[y.strftime('%Y'), tag.entries.filter(Q(account=account) & Q(day__year=y.strftime('%Y'))).aggregate(sum=Sum('amount'))['sum']] for y in years],
                'tooltip': {
                    'valueSuffix': account.unit.symbol
                },
                'type': 'column',
    ...
    if year:
        ...
        for category in Category.objects.filter(Q(entries__tags=tag) & Q(entries__account__ledgers=ledger) & Q(entries__day__year=year)).distinct():
            for unit in units:
                series.append({
                    'name': category.name,
                    'data': [[m.strftime('%B'), tag.entries.filter(Q(category=category) & Q(account__unit=unit) & Q(day__year=year) & Q(day__month=m.strftime('%m'))).aggregate(sum=Sum('amount'))['sum']] for m in months],
                    'tooltip': {
                        'valueSuffix': unit.symbol
                    },
                    'type': 'column',
        ...
        for category in Category.objects.filter(Q(entries__tags=tag) & Q(entries__account__ledgers=ledger)).distinct():
            for unit in units:
                series.append({
                    'name': category.name,
                    'data': [[y.strftime('%Y'), tag.entries.filter(Q(category=category) & Q(account__unit=unit) & Q(day__year=y.strftime('%Y'))).aggregate(sum=Sum('amount'))['sum']] for y in years],
                    'tooltip': {
                        'valueSuffix': unit.symbol
                    },
                    'type': 'column',
