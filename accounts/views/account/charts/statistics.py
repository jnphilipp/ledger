...
def categories(request, slug, year=None, month=None, category=None):
    ...
    if category and month and year:
        ...
        data = {
            ...
            "yAxis": {
                "stackLabels": {"format": "{total:,.2f}%s" % account.unit.symbol},
                "labels": {"format": "{value}%s" % account.unit.symbol},
        ...
        data = {
            ...
            "series": [
                ...
                for c in Category.objects.filter(entries__account=account).distinct()
            ],
        }
