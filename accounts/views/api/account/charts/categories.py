...
def categories(request, slug):
    ...
    data = {
        'xAxis': {
            'categories': [category.name.lower() for category in Category.objects.filter(id__in=account.entries.values_list('category', flat=True)).distinct()]},
        'series': [{
            'name': 'entries',
            'data': [[category.name.lower(), category.count] for category in Category.objects.filter(id__in=account.entries.values_list('category', flat=True)).extra(select={'count':'SELECT COUNT(*) FROM accounts_entry WHERE accounts_entry.category_id=categories_category.id AND accounts_entry.account_id=%s'}, select_params=(account.id,)).distinct()]
        }]
    }
